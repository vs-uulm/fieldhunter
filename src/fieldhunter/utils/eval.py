...
class FieldTypeReport(object):
    ...
    @staticmethod
    def newWorkbook():
        """Prepare a new workbook to hold worksheets generated by #addXLworksheet()."""
        infieldWorkbook = Workbook()
        infieldWorkbook.active.title = FieldTypeReport.ovTitle
        ovSheet = infieldWorkbook[FieldTypeReport.ovTitle]
        ovSheet.append(FieldTypeReport.overviewHeaders)
        return infieldWorkbook

    @staticmethod
    def saveWorkbook(infieldWorkbook: Workbook, pcapstrippedname: str):
        infieldFilename = join(eh.reportFolder,
                               f"FieldTypeReport_{pcapstrippedname}_{strftime('%Y%m%d-%H%M%S')}.xlsx")
